import { calculateDeltas, measureElements, type ElementSnapshot } from 'src/primitives/flipUtils';
...
describe('calculateDeltas', () => {
  it('element moved down → negative dy', () => {
    ...
    expect(deltas.get('a')).toEqual({ dx: 0, dy: -100 });
  });
  ...
  it('element moved right → negative dx', () => {
    ...
    expect(deltas.get('a')).toEqual({ dx: -50, dy: 0 });
  });
  ...
  it('element moved diagonally → both dx and dy', () => {
    ...
    expect(deltas.get('a')).toEqual({ dx: -50, dy: -100 });
  });
  ...
  it('multiple elements — only moved ones included', () => {
    ...
    expect(deltas.get('a')).toEqual({ dx: 0, dy: -100 }); // 0 → 100
    expect(deltas.get('c')).toEqual({ dx: 0, dy: 100 }); // 100 → 0
  });
  ...
  it('typical 3-item reorder: move first to last', () => {
    ...
    expect(deltas.get('b')).toEqual({ dx: 0, dy: 50 });
    // C moved from y=100 to y=50 → dy = 100 - 50 = 50
    expect(deltas.get('c')).toEqual({ dx: 0, dy: 50 });
  });
});
