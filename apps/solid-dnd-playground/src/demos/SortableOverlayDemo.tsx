...
import { createDnd, GAP_KEY, Place, type FlipAnimateEntry, type GapKey } from 'solid-dnd';
...
export default function SortableOverlayDemo(): JSX.Element {
  ...
  const dnd = createDnd({
    ...
    onDrop: (keys, place) => {
      setItems((previous) => {
        ...
        const draggedItems = previous.filter((item) => moved.has(item.id));
        const remaining = previous.filter((item) => !moved.has(item.id));

        if (place.before === null) {
          return [...remaining, ...draggedItems];
        }

        const insertionIndex = remaining.findIndex((item) => item.id === place.before);
        if (insertionIndex === -1) {
          return [...remaining, ...draggedItems];
        }

        return [...remaining.slice(0, insertionIndex), ...draggedItems, ...remaining.slice(insertionIndex)];
      });
  ...
  function layoutLabel(): string {
    return dnd.detectedLayout().mode;
  }
  ...
  return (
    <div class="flex flex-col gap-6" data-testid="sortable-overlay-demo">
      ...
      <Show when={dnd.drag.overlay.active()}>
        ...
        >
          <SortableOverlayItem items={dnd.items()} draggedIds={dnd.drag.draggedIds()} />
        </div>
      </Show>
      ...
      <OrderDisplay items={dnd.items()} columns={dnd.detectedColumns()} />
      ...
      <div class="grid grid-cols-4 gap-3">
        ...
        <StateCard
          label="dragging"
          value={dnd.isDragging() ? dnd.drag.draggedIds().join(', ') : 'none'}
          active={dnd.isDragging()}
  ...
  return (
    <div class="flex flex-wrap items-center gap-4 rounded-lg border border-white/10 bg-white/5 px-4 py-3">
      ...
      <div class="flex items-center gap-1 rounded-md border border-white/10 bg-white/5 p-0.5">
        ...
        <ToggleButton active={props.options.layout === 'horizontal'} onClick={() => props.onLayoutChange('horizontal')}>
          Horizontal
        </ToggleButton>
      ...
      <label class="flex items-center gap-2 text-xs text-neutral-400">
        ...
        >
          ...
          >
            {(easing) => <option value={easing}>{easing}</option>}
          </For>
        </select>
      </label>
