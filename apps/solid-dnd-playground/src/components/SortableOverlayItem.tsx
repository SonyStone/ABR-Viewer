 ...
 */
export function SortableOverlayItem(props: {
  ...
  const extraCount = () => props.draggedIds.length - 1;
  ...
  return (
    <div class="@container flex h-full flex-col gap-1">
      <For each={draggedItems()}>
        {(item, i) => (
          ...
          >
            ...
            {/* Color swatch */}
            <div class="h-8 w-8 shrink-0 rounded @xs:h-3 @xs:w-3 @xs:rounded-full" style={{ background: item.color }} />

            {/* Label */}
            <span class="text-xs text-neutral-200 @xs:text-sm">{item.label}</span>

            {/* ID — row mode only */}
            <span class="hidden font-mono text-xs text-neutral-500 @xs:ml-auto @xs:inline">{item.id}</span>
          </div>
        )}
      </For>
      ...
      <Show when={extraCount() > 0}>
        ...
        {/* Row mode: "N items" */}
        <div class="mt-1 hidden text-center text-xs text-blue-400/70 @xs:block">{props.draggedIds.length} items</div>
      </Show>
    </div>
  );
}
